 ...
 * Due: 09/09/2022
 * Class Descriptions:
 *    Parcel    An abstract class that defines an origin and destination address, along with a method to calculate the shipping cost.
 *    Letter    A concrete class, inheriting from Parcel, that accepts a fixed shipping cost.
 *    Address   A concrete class that stores data about a named address with an optional second address, as well as the city, state, and zipcode.
 *    GuardExtensions   Provides a useful API for throwing exceptions when evaluating the state of a given parameter.
 *    
 * Notes to Instructor:
